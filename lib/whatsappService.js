...
const initFreeClient = async (companyId, onQr, onReady, onDisconnected, forceNewSession = false) => {
  ...
  let qrTimeout = setTimeout(async () => {
    if (activeFreeClients[companyId]) {
      console.log(`[WhatsAppService] QR scan timeout for company ${companyId}. Destroying client to save resources.`);
      try {
        await client.destroy();
      } catch (err) {
        console.error("[WhatsAppService] Error destroying client on timeout:", err);
      }
      delete activeFreeClients[companyId];
      try {
        ...
        console.error("Failed to update status on timeout:", e);
      }
